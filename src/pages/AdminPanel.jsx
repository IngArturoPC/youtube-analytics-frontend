...
function AdminPanel({ user, onLogout }) {
  ...
  const navigate = useNavigate(); // <-- 2. Instancia del navegador agregada
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      fetchMetrics(); // Recargar métricas tras procesar
    } catch (err) {
      console.error(err);
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '50px auto', padding: '30px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0 / 0.1)', borderRadius: '8px', backgroundColor: '#fff', fontFamily: 'Arial, sans-serif' }}>
      
      {/* Encabezado con datos del Admin */}
      ...
      <form onSubmit={handleUpload} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        {/* Botón de Acceso Directo al Módulo de Gestión */}
        <button 
          type="button"
          onClick={() => navigate('/admin/usuarios')} 
          style={{ width: '100%', background: '#007bff', color: 'white', padding: '12px', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '10px' }}
        >
